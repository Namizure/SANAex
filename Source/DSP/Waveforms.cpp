...
float Waveforms::waveformMemory(float angle, WaveformMemoryParameters* _waveformMemoryParamsPtr, int waveformIndex) {
	checkAngleRanage(angle);
	...
	if (waveArray == nullptr) {
		return 0.0f;
	}

	std::int32_t val = *waveArray[index];
